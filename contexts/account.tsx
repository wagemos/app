...
export const AccountClientProvider: FC<
  PropsWithChildren<{ accountId: AbstractAccountId; chain: string }>
> = ({ accountId, chain, children }) => {
  const { data: abstractClient, status: abstractStatus } = useQuery({
    queryKey: ['abstract-client', chain],
    queryFn: async () =>
      AbstractQueryClient.connectToChain(chain, { useBatchClient: true }),
  })
  ...
  let value = useMemo(
    () => ({ accountId: accountId, chain, accountClient, status }),
    [accountId, chain, accountClient, status]
  )
